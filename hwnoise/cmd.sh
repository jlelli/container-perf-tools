...
if [ "${manual:-n}" == "n" ]; then
    ...
    echo "now running ..."
    python -c 'import os; maxlat=str(os.getenv("MAXLATENCY")); duration=str(os.getenv("DURATION")); cpus=os.getenv("CPUS"); maincpus=str(os.getenv("MAINCPUS")); extra=os.getenv("EXTRA"); os.system("rtla hwnoise --auto "+maxlat+" --duration "+duration+" --cpus "+cpus+" -H "+maincpus+" --quiet " + extra)'
else
    sleep infinity
fi

echo "done! if a trace was collected you can retreive it with 'oc rsync hwnoise:/root/osnoise_trace.txt .'"
